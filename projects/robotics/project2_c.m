...
% Check if the maximum acceleration is reached
if ddq_max*delta_q < ((dq_max)^2)-((dqi_motion^2 + dqf_motion^2) / 2)
    dqc = sqrt(ddq_max*delta_q+((dqi_motion^2 + dqf_motion^2) / 2));
else
    dqc = dq_max;
end
...
delta_qc = delta_q - (ddq_max * t_acc^2 + dqi_motion * t_acc) - (dqc * t_dec - ddq_max * t_dec^2);

tf = ti + t_acc + t_dec + delta_qc/dqc;
